...
export function timeUntilTakeOff(fromTime: ElfDateTime, takeOffTime: ElfDateTime): number {
	...
	const convertToUtc = (_time: ElfDateTime): string => {
		const date = _time.split("@")[0].split("*")
		const time = _time.split("@")[1].split(" ")[0].split("|")
		const result = `${date[0]}-${date[1]}-${date[2]}T${time[0]}:${time[1]}:${time[2]}Z`;
		return result;
	}

	const _fromTime = convertToUtc(fromTime)
	const _takeOffTime = convertToUtc(takeOffTime)

	const date1 = new Date(_fromTime);
	const date2 = new Date(_takeOffTime);
